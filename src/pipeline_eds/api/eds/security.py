...
def get_username(plant_name: str|None = None, overwrite: bool = False) -> str | None:
    ...
    if plant_name is None:
        return None
    username = obtain.secret(service = get_service_name(plant_name), item = "username", message = f"Enter your EDS API username for {plant_name}", overwrite=overwrite).value
    return username
