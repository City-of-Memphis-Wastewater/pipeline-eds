...
def get_username(plant_name: str|None = None, overwrite: bool = False) -> str | None:
    ...
    username = obtain.secret(service = get_service_name(plant_name), item = "username", message = f"Enter your EDS API username for {plant_name} (e.g. admin)", hide=False, overwrite=overwrite)
    return username
...
def get_password(plant_name: str|None = None, overwrite: bool = False) -> str | None:
    ...
    password = obtain.secret(service = get_service_name(plant_name), item = "password", message = f"Enter your EDS API password for {plant_name} (e.g. '')", overwrite=overwrite)
    return password
